...
public class LoginController {
    ...
    @PostMapping("/login")
    public ResponseEntity<UserResponseDto> login(@Valid @RequestBody LoginRequestDto requestDto, HttpServletRequest request) {

        UserResponseDto userIdResponseDto = userService.login(requestDto.getEmail(), requestDto.getPassword());
        UserResponseDto loginUser = userService.findUserById(userIdResponseDto.getId());
